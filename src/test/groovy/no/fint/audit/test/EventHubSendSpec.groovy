...
class EventHubSendSpec extends Specification {
    ...
    def 'Send some events'() {
        given:
        def events = [
                new Event<>(orgId: 'fintlabs.no', source: 'Spock', action: 'GET_SOMETHING', client: 'Captain Picard'),
                new Event<>(orgId: 'fintlabs.no', source: 'Frank', action: 'UPDATE_SOMETHING', client: 'Somebody to love', data: [['foo': 'bar', 'baz': 'knot']])
        ]
        expect:
        events.each {
